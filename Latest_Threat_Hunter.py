# threat_hunter.py
# Wazuh Threat Hunting Chat UI (FastAPI + WebSocket) with:
# - log source switch: alerts.json vs archives (ossec-archive-*.json)
# - agent filter: /set agent <name|all>
# - days range: /set days <n>
# - reload vector store: /reload
#
# Run (example):
#   sudo python3 /var/ossec/integrations/threat_hunter.py --days 2
#
# Notes:
# - For best "latest events", use: /set source alerts  (then /reload)
# - To focus a Linux agent named "payment", use: /set agent payment (then /reload)
...
# defaults (can be changed via chat commands)
days_range = 2
agent_filter = "all"       # e.g. "payment" or "Lenovo-W11" or "all"
log_source = "alerts"      # "alerts" or "archives"

username = "admin"
password = "admin"

# If you want remote loading (optional)
...
# =========================
# Helpers: timestamps / formatting
# =========================

def parse_wazuh_timestamp(ts: str):
    """
    Parse common Wazuh timestamp formats.
    ...
    # Try ISO formats: 2026-03-05T01:23:45.123+0000 or ...Z
    try:
        # normalize Z
        if ts.endswith("Z"):
...
def format_history(history_pairs, max_turns=12):
    """
    history_pairs: list of (role, text) where role in {"user","bot"}
    """
...
def event_to_text(evt: dict) -> str:
    """
    Convert a Wazuh event/alert dict into a searchable text block.
    Works for alerts.json and archives.
    """
...
def load_alerts_from_days(past_days=1):
    """
    Load Wazuh alerts from /var/ossec/logs/alerts/alerts.json (recent N days).
    This is the recommended source for "latest security events".
    """
    ...
    with open(base, "rt", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if t:
                # If tz-naive, assume UTC to be safe
                if t.tzinfo is None:
...
def load_archives_from_days(past_days=7):
    """
    Load raw archives from /var/ossec/logs/archives/<year>/<Mon>/ossec-archive-<dd>.json(.gz)
    """
    if remote_host:
...
def load_logs_from_days(past_days=7):
    """
    Dispatcher based on log_source global:
    - alerts: /var/ossec/logs/alerts/alerts.json
    - archives: /var/ossec/logs/archives/... ossec-archive-*.json(.gz)
    """
...
def initialize_assistant_context():
    return (
        ...
        "You analyze Wazuh events provided as context (alerts or archives depending on the selected source).\n"
        "Identify security threats, suspicious patterns, and answer user questions using the retrieved Wazuh context.\n"
        "If the logs do not contain enough info, say what is missing and what to check next.\n"
        "When asked, summarize by: affected agents, usernames, source IPs, timeframe, and suggested next steps."
    )
...
def setup_chain(past_days=7):
    ...
    print("📦 Creating vectorstore (this may take time the first run)...")
...
def get_stats(logs):
    total = len(logs)
    # Attempt a basic timestamp range from parsed timestamps
    parsed = []
    for evt in logs:
        ts = evt.get("timestamp") or evt.get("@timestamp") or ""
        t = parse_wazuh_timestamp(ts)
...
chat_history = []  # list of tuples: ("user"|"bot", "message")
        ...
        while True:
            ...
            if low.startswith("/set agent"):
                try:
                    ...
                    await websocket.send_json({"role": "bot", "message": f"✅ agent filter set to '{agent_filter}' (effective on next /reload)."})
                except Exception:
                    await websocket.send_json({"role": "bot", "message": "⚠️ Usage: /set agent <name|all>"})
                continue
            ...
            if low.startswith("/set source"):
                try:
                    ...
                    await websocket.send_json({"role": "bot", "message": f"✅ log source set to '{log_source}' (effective on next /reload)."})
                except Exception:
                    await websocket.send_json({"role": "bot", "message": "⚠️ Usage: /set source alerts|archives"})
                continue
