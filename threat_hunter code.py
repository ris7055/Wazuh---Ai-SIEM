                ...
                try:
                    for line in remote_file:
                        ...
                        try:
                            ...
                            print("⚠️ Skipping invalid JSON line from remote file.")
                except Exception as e:
...
def create_vectorstore(logs, embedding_model):
    ...
    for log in logs:
        full = log.get("full_log", "") or ""
        if not full.strip():
            continue
        splits = text_splitter.split_text(full)
        for chunk in splits:
...
def initialize_assistant_context():
    return (
        ...
        "You analyze Wazuh logs provided as context.\n"
        "Identify security threats, suspicious patterns, and answer user questions using the retrieved log context.\n"
...
def setup_chain(past_days=7):
    ...
    print(f"🔄 Initializing retrieval chain with logs from past {past_days} days...")
    ...
    # Modern prompt + retrieval chain
    prompt = ChatPromptTemplate.from_messages([
        ("system", context),
        ("human",
         "Chat history:\n{chat_history}\n\n"
         "User question: {input}\n\n"
         "Use the following retrieved Wazuh log context to answer:\n{context}\n")
    ])
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({
            "role": "bot",
            "message": f"👋 Hello! Ask me anything about Wazuh logs.\n(Default date range is {days_range} days.)\nType /help for commands."
        })
